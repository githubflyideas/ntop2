...
// Linux 上编译得过的文件,而这会连带把 cmd/ntop2 整个拖下水——哪怕
// Mac 用户只想收 NetFlow、根本不碰本机抓包。这种破坏在 Linux 上跑
// go build ./... 是发现不了的,所以必须显式跨编译一次。
//
// 编译目标是 cmd/ntop2 而不是本包:约束的实质是"程序能在 Mac 上出
...
func TestBuildsForDarwin(t *testing.T) {
	...
	for _, arch := range []string{"arm64", "amd64"} {
		t.Run(arch, func(t *testing.T) {
			cmd := exec.Command("go", "build", "-o", t.TempDir()+"/ntop2",
				"github.com/githubflyideas/ntop2ban/cmd/ntop2")
