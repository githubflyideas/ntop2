...
// decodeQuery 解出 Query AST。
//
// 多包一层而不是直接 Decode 到 query.Query:请求体里出现未知字段时,
// 错误信息里要带上"AST 格式错误"这几个字,否则界面上只有一句 JSON 库的
// 英文报错,看不出错在哪一层。
func decodeQuery(r *http.Request) (query.Query, error) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		return query.Query{}, fmt.Errorf("AST 格式错误: %w", err)
	}
